        ...
        <div class="col-md-3 d-flex flex-column p-0 sidebar">
          <div class="sidebar-content flex-grow-1 d-flex flex-column justify-content-between">
            ...
            <div class="text-center py-4">
              ...
              <h2 class="titulo-sidebar">Responsables</h2>
            </div>
            <!-- Opciones -->
            <div class="enlaces-centrales">
              <ul class="list-unstyled">
                ...
                <li class="opcion-link">
                  <router-link :to="{name:'insertarR'}" class="enlace-menu">
                    <span>Nuevo Responsable</span>
                  </router-link>
                </li>
                <li class="opcion-link">
                  <a href="#" @click.prevent="consultarResponsables" class="enlace-menu">
        ...
        <div class="col-md-9 d-flex flex-column p-4">
          <div class="card shadow flex-grow-1">
            <div color="#446b9b" class="card-header text-center">
              <h3 class="mb-0">Listado de Responsables</h3>
            </div>
            <div class="card-body tabla-scroll">
              ...
              <div class="table-responsive tabla-contenedor">
                <table class="table table-bordered table-hover align-middle">
                  ...
                  <tbody>
                    <tr v-for="responsable in responsablesFiltrados" :key="responsable.id" class="fila-interactiva">
                      ...
                      <td>
                        <div class="btn-group">
                          <router-link :to="{name:'editarR', params:{id: responsable.id}}" class="btn btn-warning btn-sm">Editar</router-link>
                          <button type="button" @click="borrarResponsable(responsable.codigoAsignado)" class="btn btn-danger btn-sm">Eliminar</button>
                        </div>
                      </td>
                    </tr>
                    <tr v-if="responsablesFiltrados.length === 0">
                      <td colspan="6" class="text-center text-muted">No hay responsables registrados</td>
                    </tr>
                  </tbody>
                </table>
              </div>
...
export default {
    ...
    computed: {
        responsablesFiltrados() {
            ...
            if (!this.filtroActivo) return this.responsables

            return this.responsables.filter(u =>
    ...
    methods: {
        consultarResponsables() {
            ...
            fetch('http://localhost/sgt/IngSoftware_Tareas/SISTEMA/APIS/Responsables.php')
                .then(res => res.json())
                .then(data => {
        ...
        borrarResponsable(codigoAsignado) {
            if (!confirm("¿Estás seguro de que deseas eliminar este responsable?")) {
                return;
            }

            // CLAVE: Usamos codigoAsignado en la URL
            fetch('http://localhost/sgt/IngSoftware_Tareas/SISTEMA/APIS/Responsables.php?borrar=' + codigoAsignado)
                .then(res => res.json())
                .then(data => {
                    if (data.success === 1) {
                        // Éxito:
                        alert(data.message || "Responsable eliminado correctamente.");
                        this.consultarResponsables();
                    } else {
                ...
                .catch(err => {
                    console.error('Error al intentar eliminar responsable:', err);
